<?php
class Wsu_WebForms_Adminhtml_ResultsController extends Mage_Adminhtml_Controller_Action {
	...
	public function editAction() {
		$this->_initAction();
		$this->getLayout()->getBlock('content')->append($this->getLayout()->createBlock('webforms/adminhtml_results_edit', 'edit'));
		$this->renderLayout();
	}
	public function saveAction() {
		...
		if ($webformId) {
			...
			$resultId = $webform->savePostResult(array(
				'prefix' => 'result'
			));
			...
			if (!$resultId) {
				if ($postData['result_id']) {
					...
					$this->_redirect('*/adminhtml_results/edit', array(
						'id' => $resultId
					));
					return;
				}
				$this->_redirect('*/adminhtml_results/index', array(
	...
	public function replyAction() {
		$this->_initAction();
		$this->getLayout()->getBlock('content')->append($this->getLayout()->createBlock('webforms/adminhtml_reply', 'reply'));
		$this->renderLayout();
	}
	public function saveMessageAction() {
		...
		foreach ($Ids as $id) {
			...
			$message = Mage::getModel('webforms/message')->setMessage($message)->setAuthor($user->getName())->setUserId($user->getId())->setResultId($id)->save();
			if ($post['email']) {
	...
	public function gridAction() {
		...
		if (!Mage::registry('webform_data')) {
			$webform = Mage::getModel('webforms/webforms')->setStoreId($this->getRequest()->getParam('store'))->load($this->getRequest()->getParam('webform_id'));
			Mage::register('webform_data', $webform);
		}
		$this->getResponse()->setBody($this->getLayout()->createBlock('webforms/adminhtml_results_grid')->toHtml());
	}
	public function deleteAction() {
		if ($this->getRequest()->getParam('id') > 0) {
			try {
				...
				$this->_redirect('*/*/', array(
					'webform_id' => $result->getWebformId()
				));
			}
			catch (Exception $e) {
	...
	public function exportCsvAction() {
		...
		$content  = $this->getLayout()->createBlock('webforms/adminhtml_results_grid')->getCsvFile();
	...
	public function exportXmlAction() {
		...
		$content  = $this->getLayout()->createBlock('webforms/adminhtml_results_grid')->getExcelFile();
		$this->_prepareDownloadResponse($fileName, $content);
	}
	public function massEmailAction() {
		...
		try {
			...
			$this->_getSession()->addSuccess($this->__('Total of %d result(s) have been emailed.', count($k)));
		}
	...
	public function massApproveAction($approveStatus = 1) {
		...
		try {
			foreach ($Ids as $id) {
				...
				$result->save();
				Mage::dispatchEvent('webforms_result_approve', array(
					'result' => $result
				));
			}
			$this->_getSession()->addSuccess($this->__('Total of %d result(s) have been updated.', count($Ids)));
		}
		catch (Mage_Core_Model_Exception $e) {
			$this->_getSession()->addError($e->getMessage());
		}
