<?php
class Wsu_WebForms_Adminhtml_LogicController extends Mage_Adminhtml_Controller_Action {
	protected function _initObjects() {
		...
		if ($field->getWebformId())
			$webform->load($field->getWebformId());
	...
	public function gridAction() {
		$this->loadLayout();
		$this->_initObjects();
		$this->getResponse()->setBody($this->getLayout()->createBlock('webforms/adminhtml_fields_edit_tab_logic')->toHtml());
	}
	...
	public function saveAction() {
		if ($this->getRequest()->getPost()) {
			try {
				...
				if ($saveandcontinue) {
					$this->_redirect('*/adminhtml_logic/edit', array(
						'id' => $logic->getId(),
					...
					if ($this->getRequest()->getParam('webform_id')) {
						$this->_redirect('*/adminhtml_webforms/edit', array(
							'id' => $this->getRequest()->getParam('webform_id'),
							'tab' => 'logic',
							'store' => $store
						));
					} else
						$this->_redirect('*/adminhtml_fields/edit', array(
							'id' => $field_id,
							'tab' => 'logic',
							'store' => $store
						));
	...
	public function massDeleteAction() {
		...
		if ($this->getRequest()->getParam('webform_id')) {
			$this->_redirect('webforms/adminhtml_webforms/edit', array(
				'id' => $this->getRequest()->getParam('webform_id'),
				'tab' => 'logic',
				'store' => $store
			));
		...
		$this->_redirect('webforms/adminhtml_fields/edit', array(
