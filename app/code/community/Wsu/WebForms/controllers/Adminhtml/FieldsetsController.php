<?php
class Wsu_WebForms_Adminhtml_FieldsetsController extends Mage_Adminhtml_Controller_Action {
	...
	public function gridAction() {
		$this->loadLayout();
		$this->getResponse()->setBody($this->getLayout()->createBlock('webforms/adminhtml_webforms_edit_tab_fieldsets')->toHtml());
	}
	...
	public function saveAction() {
		if ($this->getRequest()->getPost()) {
			try {
				...
				if ($saveandcontinue) {
					$this->_redirect('*/adminhtml_fieldsets/edit', array(
						'id' => $fieldset->getId(),
	...
	public function massDuplicateAction() {
		...
		try {
			...
			$this->_getSession()->addSuccess($this->__('Total of %d record(s) have been duplicated.', count($Ids)));
		}
		catch (Mage_Core_Model_Exception $e) {
			$this->_getSession()->addError($e->getMessage());
		}
