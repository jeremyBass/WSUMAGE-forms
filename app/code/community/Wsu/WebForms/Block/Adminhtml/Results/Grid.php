<?php
class Wsu_WebForms_Block_Adminhtml_Results_Grid extends Mage_Adminhtml_Block_Widget_Grid {
	...
	protected function _filterCustomerCondition($collection, $column) {
		...
		$customers       = Mage::getModel('customer/customer')->getCollection()->addAttributeToFilter('firstname', $firstname);
		if (count($name) == 2)
	...
	protected function _prepareCollection() {
		...
		Mage::dispatchEvent('webforms_block_adminhtml_results_grid_prepare_collection', array(
			'grid' => $this
		));
		return parent::_prepareCollection();
	}
	protected function _prepareColumns() {
		$renderer = 'Wsu_WebForms_Block_Adminhtml_Results_Renderer_Id';
		if ($this->_isExport) {
			$renderer = false;
		}
		...
		$fields = Mage::getModel('webforms/fields')->setStoreId($this->getRequest()->getParam('store'))->getCollection()->addFilter('webform_id', $this->getRequest()->getParam('webform_id'));
			...
			if ($field->getType() != 'html') {
				...
				$config = array(
					...
					'filter_condition_callback' => array(
						$this,
						'_filterFieldCondition'
					),
					'renderer' => 'Wsu_WebForms_Block_Adminhtml_Results_Renderer_Value'
				);
				if ($this->_isExport) {
				...
				Mage::dispatchEvent('webforms_block_adminhtml_results_grid_prepare_columns_config', array(
					'field' => $field,
					'config' => $config
				));
		...
		$config = array(
			...
			'align' => 'left',
			'index' => 'customer_id',
			'renderer' => 'Wsu_WebForms_Block_Adminhtml_Results_Renderer_Customer',
			'filter_condition_callback' => array(
				$this,
				'_filterCustomerCondition'
			),
			'sortable' => false
		);
		if ($this->_isExport) {
		...
		$this->addColumn('action', array(
			...
			'filter' => false,
			'sortable' => false,
			'renderer' => 'Wsu_WebForms_Block_Adminhtml_Results_Renderer_Action',
			'is_system' => true
		));
		$this->addExportType('*/*/exportCsv', Mage::helper('webforms')->__('CSV'));
		$this->addExportType('*/*/exportXml', Mage::helper('webforms')->__('Excel XML'));
		Mage::dispatchEvent('webforms_block_adminhtml_results_grid_prepare_columns', array(
			'grid' => $this
		));
	...
	protected function _prepareMassaction() {
		...
		$this->getMassactionBlock()->addItem('email', array(
			...
			'url' => $this->getUrl('*/*/massEmail', array(
				'webform_id' => $this->getRequest()->getParam('webform_id')
			)),
		...
		$this->getMassactionBlock()->addItem('reply', array(
			...
			'url' => $this->getUrl('*/*/reply', array(
				'webform_id' => $this->getRequest()->getParam('webform_id')
			)),
			'confirm' => Mage::helper('webforms')->__('Reply to selected results?')
		));
		Mage::dispatchEvent('webforms_adminhtml_results_grid_prepare_massaction', array(
			'grid' => $this
		));
		return $this;
	}
