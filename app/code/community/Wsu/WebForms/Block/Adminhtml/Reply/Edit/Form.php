<?php
class Wsu_WebForms_Block_Adminhtml_Reply_Edit_Form extends Mage_Adminhtml_Block_Widget_Form {
		...
		if (count($Ids) == 1) {
			...
			if (count($history)) {
				$form->addField('reply_history', 'note', array(
					"text" => ' <div class="entry-edit-head"><h4 class="icon-head head-edit-form fieldset-legend">' . Mage::helper('webforms')->__('Messages History') . '</h4></div><div class="fieldset"><div class="hor-scroll">' . $this->getLayout()->createBlock('webforms/adminhtml_reply_history')->toHtml() . '</div></div>'
				));
			}
		}
