<?php
class Wsu_WebForms_Block_Results extends Wsu_WebForms_Block_Webforms implements Mage_Widget_Block_Interface {
	...
	public function getResultsCollection() {
		if (null === $this->_resultsCollection) {
			$data                     = $this->getFormData();
			$this->_resultsCollection = Mage::getModel('webforms/results')->getCollection()->addFilter('store_id', Mage::app()->getStore()->getId())->addFilter('webform_id', $data["webform_id"])->addFilter('approved', 1);
