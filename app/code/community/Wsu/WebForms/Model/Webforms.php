...
class Wsu_WebForms_Model_Webforms extends Wsu_WebForms_Model_Abstract {
	...
	public function getAvailableStatuses() {
		...
		Mage::dispatchEvent('webforms_statuses', array(
			'statuses' => $statuses
		));
	...
	public function getFieldsetsOptionsArray() {
		$collection = Mage::getModel('webforms/fieldsets')->setStoreId($this->getStoreId())->getCollection()->addFilter('webform_id', $this->getId());
	...
	public function getTemplatesOptions() {
		$options   = array(
			0 => Mage::helper('webforms')->__('Default')
		);
	...
	public function getFieldsToFieldsets($all = false) {
		...
		//get form fieldsets
		$fieldsets   = Mage::getModel('webforms/fieldsets')->setStoreId($this->getStoreId())->getCollection()->addFilter('webform_id', $this->getId());
		if (!$all)
		...
		foreach ($fields as $field) {
			...
			$field->setData('logic_visibility', Wsu_WebForms_Model_Logic::VISIBILITY_VISIBLE);
		...
		foreach ($fields_to_fieldsets as $fieldset_id => $fieldset) {
			$fields_to_fieldsets[$fieldset_id]['logic_visibility'] = Wsu_WebForms_Model_Logic::VISIBILITY_VISIBLE;
			if (count($logic_rules))
				foreach ($logic_rules as $logic) {
					if ($logic->getAction() == Wsu_WebForms_Model_Logic_Action::ACTION_SHOW && $logic->getIsActive()) {
						// check fieldset visibility
						if (in_array('fieldset_' . $fieldset_id, $logic->getTarget())) {
							$fields_to_fieldsets[$fieldset_id]['logic_visibility'] = Wsu_WebForms_Model_Logic::VISIBILITY_HIDDEN;
							$hidden_targets[]                                      = "fieldset_" . $fieldset_id;
						}
						// check fields visibility
						foreach ($fieldset['fields'] as $field) {
							if (in_array('field_' . $field->getId(), $logic->getTarget())) {
								$field->setData('logic_visibility', Wsu_WebForms_Model_Logic::VISIBILITY_HIDDEN);
		...
		foreach ($fields_to_fieldsets as $fieldset_id => $fieldset) {
			...
			$fields_to_fieldsets[$fieldset_id]['logic_visibility'] = $visibility ? Wsu_WebForms_Model_Logic::VISIBILITY_VISIBLE : Wsu_WebForms_Model_Logic::VISIBILITY_HIDDEN;
			// check fields visibility
			foreach ($fieldset['fields'] as $field) {
				$target['id']               = 'field_' . $field->getId();
				$target['logic_visibility'] = $field->getData('logic_visibility');
				$visibility                 = $logicModel->getTargetVisibility($target, $logic_rules, $default_data);
				$field->setData('logic_visibility', $visibility ? Wsu_WebForms_Model_Logic::VISIBILITY_VISIBLE : Wsu_WebForms_Model_Logic::VISIBILITY_HIDDEN);
			}
		}
		// set logic target
		foreach ($logic_rules as $logic)
			if ($logic->getIsActive())
				foreach ($logic->getTarget() as $target) {
					...
					if (!in_array($target, $logic_target))
						$logic_target[] = array(
							"id" => $target,
							"logic_visibility" => in_array($target, $hidden_targets) ? Wsu_WebForms_Model_Logic::VISIBILITY_HIDDEN : Wsu_WebForms_Model_Logic::VISIBILITY_VISIBLE,
							"required" => $required
						);
				}
	...
	public function useCaptcha() {
		$useCaptcha = Mage::helper('webforms')->captchaAvailable();
		if ($useCaptcha) {
			$captcha_mode = $this->getCaptchaMode();
			if( $captcha_mode == "off" 
			||	Mage::getSingleton('customer/session')->getCustomerId() && $captcha_mode == "auto" 
			||	$this->getData('disable_captcha') 
			){
				$useCaptcha = false;
			}	
		}
	...
	public function duplicate() {
		// duplicate form
		$form   = Mage::getModel('webforms/webforms')->setData($this->getData())->setId(null)->setName($this->getName() . ' ' . Mage::helper('webforms')->__('(new copy)'))->setIsActive(false)->setCreatedTime(Mage::getSingleton('core/date')->gmtDate())->setUpdateTime(Mage::getSingleton('core/date')->gmtDate())->save();
		// duplicate store data
		$stores = Mage::getModel('webforms/store')->getCollection()->addFilter('entity_id', $this->getId())->addFilter('entity_type', $this->getEntityType());
		foreach ($stores as $store) {
			Mage::getModel('webforms/store')->setData($store->getData())->setId(null)->setEntityId($form->getId())->save();
		}
		...
		foreach ($fields_to_fieldsets as $fieldset_id => $fieldset) {
			if ($fieldset_id) {
				...
				$new_fieldset                  = Mage::getModel('webforms/fieldsets')->setData($fs->getData())->setId(null)->setCreatedTime(Mage::getSingleton('core/date')->gmtDate())->setUpdateTime(Mage::getSingleton('core/date')->gmtDate())->setWebformId($form->getId())->save();
				$new_fieldset_id               = $new_fieldset->getId();
				$fieldset_update[$fieldset_id] = $new_fieldset_id;
				// duplicate store data
				$stores                        = Mage::getModel('webforms/store')->getCollection()->addFilter('entity_id', $fs->getId())->addFilter('entity_type', $fs->getEntityType());
				foreach ($stores as $store) {
					Mage::getModel('webforms/store')->setData($store->getData())->setId(null)->setEntityId($new_fieldset_id)->save();
				}
			} else {
				$new_fieldset_id = 0;
			}
			foreach ($fieldset['fields'] as $field) {
				$new_field                     = Mage::getModel('webforms/fields')->setData($field->getData())->setId(null)->setCreatedTime(Mage::getSingleton('core/date')->gmtDate())->setUpdateTime(Mage::getSingleton('core/date')->gmtDate())->setWebformId($form->getId())->setFieldsetId($new_fieldset_id)->save();
				$field_update[$field->getId()] = $new_field->getId();
				// duplicate store data
				$stores                        = Mage::getModel('webforms/store')->getCollection()->addFilter('entity_id', $field->getId())->addFilter('entity_type', $field->getEntityType());
				foreach ($stores as $store) {
					Mage::getModel('webforms/store')->setData($store->getData())->setId(null)->setEntityId($new_field->getId())->save();
				}
			}
		}
		// duplicate logic
		...
		foreach ($logic_rules as $logic) {
			...
			$new_logic = Mage::getModel('webforms/logic')->setData($logic->getData())->setId(null)->setCreatedTime(Mage::getSingleton('core/date')->gmtDate())->setUpdateTime(Mage::getSingleton('core/date')->gmtDate())->setFieldId($new_field_id)->setTarget($new_target)->save();
			// duplicate store data
			$stores    = Mage::getModel('webforms/store')->getCollection()->addFilter('entity_id', $logic->getId())->addFilter('entity_type', $logic->getEntityType());
			foreach ($stores as $store) {
				$new_target = array();
				...
				Mage::getModel('webforms/store')->setData($store->getData())->setId(null)->setEntityId($new_logic->getId())->save();
			}
		}
		return $form;
	}
	...
	public function validatePostResult() {
				...
				if ($this->getEmailSmtpValidation()) {
					if ($field->getIsActive() && $field->getType() == 'email') {
						if (!empty($postData[$field->getId()])) {
							...
							$smtp_validate_results = $smtp_validate->validate(array(
								$postData[$field->getId()]
		...
		$validate = new Varien_Object(array(
			'errors' => $errors
		));
	...
	public function savePostResult($config = array()) {
		try {
			...
			$iplong = ip2long(Mage::helper('webforms')->getIp());
			$files  = $this->getUploadedFiles();
			foreach ($files as $field_name => $file) {
				$field_id = str_replace('file_', '', $field_name);
				if ($file['name']) {
