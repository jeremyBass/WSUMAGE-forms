<?php
class Wsu_WebForms_Model_Fields extends Wsu_WebForms_Model_Abstract {
	...
	public function getFieldTypes() {
		...
		Mage::dispatchEvent('webforms_fields_types', array(
			'types' => $types
		));
	...
	public function getResultsOptions() {
		$query   = $this->getResource()->getReadConnection()->select('value')->from($this->getResource()->getTable('webforms/results_values'), array(
			'value'
		))->where('field_id = ' . $this->getId())->order('value asc')->distinct();
	...
	public function getSizeTypes() {
		...
		// add more size types
		Mage::dispatchEvent('webforms_fields_size_types', array(
			'types' => $types
		));
		return $types->getData();
	}
	...
	public function getOptionsArray() {
		...
		foreach ($values as $val) {
			...
			if (strlen(trim($val)) > 0) {
				...
				$label = $value;
				if (Mage::getStoreConfig('webforms/general/use_translation'))
					$label = Mage::helper('webforms')->__($value);
				$matches = array();
				preg_match($this->val_regex, $value, $matches);
				if (!empty($matches[1])) {
					$value = trim($matches[1]);
	...
	public function toHtml() {
		...
		// apply custom filter
		Mage::dispatchEvent('webforms_fields_tohtml_filter', array(
			'filter' => $filter
		));
		...
		if ($tinyMCE && !Mage::registry('tinyMCE')) {
			...
			$tiny_mce = $layout->createBlock('core/template', 'tinyMCE', array(
				'template' => 'webforms/scripts/tiny_mce.phtml'
			));
		...
		$html_object = new Varien_Object(array(
			'html' => $html
		));
	...
	public function duplicate() {
		// duplicate field
		$field  = Mage::getModel('webforms/fields')->setData($this->getData())->setId(null)->setName($this->getName() . ' ' . Mage::helper('webforms')->__('(new copy)'))->setIsActive(false)->setCreatedTime(Mage::getSingleton('core/date')->gmtDate())->setUpdateTime(Mage::getSingleton('core/date')->gmtDate())->save();
		...
		foreach ($stores as $store) {
			$duplicate = Mage::getModel('webforms/store')->setData($store->getData())->setId(null)->setEntityId($field->getId())->save();
		}
		return $field;
	}
			...
			if ($fieldset_id) {
				if ($this->getFieldsetId() != $fieldset_id)
					$options[] = array(
						...
						'label' => $fieldset['name'] . ' [' . Mage::helper('webforms')->__('Field Set') . ']'
					);
