...
class Wsu_WebForms_Model_Observer {
	...
	public function adjustConfig($observer) {
		if (Mage::registry('webforms_adjust_config'))
			return false;
		//adjust acl
		$parent = Mage::getSingleton('admin/config')->getAdminhtmlConfig()->getNode('acl')->descend('resources')->descend('admin')->descend('children')->descend('webforms');
		if ($parent) {
			$webformsNode = $parent->descend('children');
			if ($webformsNode) {
				$collection = Mage::getModel('webforms/webforms')->setStoreId(Mage::app()->getStore()->getId())->getCollection()->addFilter('menu', '1');
				...
				foreach ($collection as $webform) {
					...
					if ($menuitem)
						$webformsNode->appendChild($menuitem);
				}
			}
		}
		//adjust menu
		$parent = Mage::getSingleton('admin/config')->getAdminhtmlConfig()->getNode('cms');
		if ($parent) {
			...
			if ($webformsNode) {
				...
				');
				if ($settings)
					$webformsNode->appendChild($settings);
			}
		}
