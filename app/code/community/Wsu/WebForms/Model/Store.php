<?php
class Wsu_WebForms_Model_Store extends Mage_Core_Model_Abstract {
	...
	public function search($store_id, $entity_type, $entity_id) {
		...
		$select = $read->select()->from($this->getResource()->getMainTable(), array(
			'id'
		))->where('store_id=?', $store_id)->where('entity_type=?', $entity_type)->where('entity_id=?', $entity_id);
