<?php
class Wsu_WebForms_Model_Mysql4_Results_Collection extends Mage_Core_Model_Mysql4_Collection_Abstract {
	...
	protected function _afterLoad() {
		...
		foreach ($this as $item) {
			$query   = $this->getConnection()->select()->from($this->getTable('webforms/results_values'))->where($this->getTable('webforms/results_values') . '.result_id = ' . $item->getId());
		...
		Mage::dispatchEvent('webforms_results_collection_load', array(
			'collection' => $this
		));
		return $this;
	}
	public function addFieldFilter($field_id, $value) {
		...
		if (is_array($value)) {
			if (strstr($field->getType(), 'date')) {
				if ($value['from'])
					$value['from'] = "'" . date($field->getDbDateFormat(), strtotime($value['orig_from'])) . "'";
				if ($value['to'])
					$value['to'] = "'" . date($field->getDbDateFormat(), strtotime($value['orig_to'])) . "'";
			}
			if ($value['from']) {
		...
		$this->getSelect()->join(array(
			'results_values_' . $field_id => $this->getTable('webforms/results_values')
		), 'main_table.id = results_values_' . $field_id . '.result_id', array(
			'main_table.*'
		))->group('main_table.id');
		$this->getSelect()->where("results_values_$field_id.field_id = $field_id AND $cond");
	}
}