<?php
class Wsu_WebForms_Model_Mysql4_Logic_Collection extends Mage_Core_Model_Mysql4_Collection_Abstract {
	...
	public function addWebformFilter($webform_id) {
		...
		), 'main_table.field_id = fields.id', array(
			'main_table.*',
			'fields.name',
			'fields.webform_id'
		))->group('main_table.id');
		$this->getSelect()->where("fields.webform_id = '{$webform_id}'");
		return $this;
	}
}