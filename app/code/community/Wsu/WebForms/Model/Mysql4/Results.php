<?php
class Wsu_WebForms_Model_Mysql4_Results extends Mage_Core_Model_Mysql4_Abstract {
	...
	protected function _afterSave(Mage_Core_Model_Abstract $object) {
		...
		Mage::dispatchEvent('webforms_result_save', array(
			'result' => $object
		));
	...
	protected function _afterLoad(Mage_Core_Model_Abstract $object) {
		...
		Mage::dispatchEvent('webforms_result_load', array(
			'webform' => $webform,
			'result' => $object
		));
	...
	protected function _afterDelete(Mage_Core_Model_Abstract $object) {
		...
		foreach ($messages as $message)
			$message->delete();
		Mage::dispatchEvent('webforms_result_delete', array(
			'result' => $object
		));
	...
	public function getSummaryRatings($webform_id, $store_id) {
		...
		$select      = $adapter->select()->from(array(
			'results_values' => $this->getTable('webforms/results_values')
		), array(
			'sum' => $sumColumn,
			'count' => $countColumn,
			'field_id'
		))->join(array(
			'fields' => $this->getTable('webforms/fields')
		), 'results_values.field_id = fields.id', array())->join(array(
			'results' => $this->getTable('webforms/results')
		), 'results_values.result_id = results.id', array())->where('fields.type = "stars"')->where('results.webform_id = ' . $webform_id)->where('results.store_id = ' . $store_id)->where('results.approved = 1')->group('results_values.field_id');
