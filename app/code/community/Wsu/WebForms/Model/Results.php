<?php
class Wsu_WebForms_Model_Results extends Mage_Core_Model_Abstract {
	...
	public function sendEmail($recipient = 'admin', $contact = false) {
		...
		if (strstr(strval($email), ',') && $recipient == 'admin') {
			...
			foreach ($email_array as $email) {
				$success = Mage::getModel('core/email_template')->setTemplateSubject($subject)->setReplyTo($this->getReplyTo($recipient))->sendTransactional($templateId, $sender, trim($email), $name, $vars, $storeId);
			}
		} else {
			$success = Mage::getModel('core/email_template')->setTemplateSubject($subject)->setReplyTo($this->getReplyTo($recipient))->sendTransactional($templateId, $sender, $email, $name, $vars, $storeId);
		}
		return $success;
	}
	public function getEmailSubject($recipient = 'admin') {
		...
		$subject_array = array();
		$fields        = Mage::getModel('webforms/fields')->setStoreId($this->getStoreId())->getCollection()->addFilter('webform_id', $this->getWebformId())->addFilter('email_subject', 1);
	...
	public function getCustomerName() {
		$customer_name = array();
		$fields        = Mage::getModel('webforms/fields')->setStoreId($this->getStoreId())->getCollection()->addFilter('webform_id', $this->getWebformId());
		foreach ($fields as $field) {
			foreach ($this->getData() as $key => $value) {
				if (is_string($value))
					$value = trim($value);
	...
	public function getContactArray() {
		$fields = Mage::getModel('webforms/fields')->setStoreId($this->getStoreId())->getCollection()->addFilter('webform_id', $this->getWebformId())->addFilter('type', 'select/contact');
		foreach ($fields as $field) {
			foreach ($this->getData() as $key => $value) {
				if ($key == 'field_' . $field->getId() && $value) {
	...
	public function getReplyTo($recipient = 'admin') {
		$fields   = Mage::getModel('webforms/fields')->setStoreId($this->getStoreId())->getCollection()->addFilter('webform_id', $this->getWebformId())->addFilter('type', 'email');
		$webform  = Mage::getModel('webforms/webforms')->setStoreId($this->getStoreId())->load($this->getWebformId());
		$reply_to = false;
		foreach ($this->getData() as $key => $value) {
	...
	public function toHtml($recipient = 'admin', $options = array()) {
		...
		$fields_to_fieldsets = Mage::getModel('webforms/webforms')->setStoreId($this->getStoreId())->load($this->getWebformId())->getFieldsToFieldsets(true);
	...
	public function toXml(array $arrAttributes = array(), $rootName = 'item', $addOpenTag = false, $addCdata = true) {
		...
		foreach ($this->getData() as $key => $value) {
			if (strstr($key, 'field_')) {
				$field = Mage::getModel('webforms/fields')->setStoreId($this->getStoreId())->load(str_replace('field_', '', $key));
